...
if(!$kdao_only){//只使用於綜合網址
	...
	//過濾
	$pattern="%(method=POST>.*</blockquote>)%U";//非貪婪
	preg_match_all($pattern, $content, $matches_a);//內文-首篇
	//print_r($matches_a);//
	if(count($matches_a[1])==0){die("[x]沒找到首篇內文");}//沒找到
	//過濾
	...
	*/
	...
	foreach($matches_ab as $k => $v){//迴圈
		...
		*/
		...
		$matches_msg[1]=內文
		*/
		$pattern="%<font color='#cc1105'><b>(.+)</b></font>(.+)<a class=del%U";//名稱 ID時間
...
if($w_chk){//寫入到檔案
    ...
    $output.=htmlend();
    //
    $logfile=$dir_mth."ch".$no.".htm";//接頭(prefix)接尾(suffix)
